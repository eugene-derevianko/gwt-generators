...
public class CellInfoExtractor extends AbstractExtractor
{
   ...
   private JType decideReturnType(JClassType cellType, JType returnType)
   {
      ...
      for (JClassType stype : cellType.getFlattenedSupertypeHierarchy())
      {
         if (cellClassName.equals(stype.getQualifiedSourceName()))
         {
            JParameterizedType ptype = stype.isParameterized();
            if (ptype != null)
            {
               return ptype.getTypeArgs()[0];
            }
         }
      }
      return returnType;
